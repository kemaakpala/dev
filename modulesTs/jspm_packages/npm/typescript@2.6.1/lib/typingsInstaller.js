...
(function(process) {
  ...
  (function(ts) {
    ...
    function normalizePathAndParts(path) {
      ...
      if (parts.length) {
        ...
        return {
          path: pathEndsWithDirectorySeparator(path) ? joinedParts + ts.directorySeparator : joinedParts,
          parts: parts
        };
      } else {
        return {
    ...
    function createCachedDirectoryStructureHost(host) {
      ...
      var getCurrentDirectory = memoize(function() {
      ...
      return {
        ...
        readFile: function(path, encoding) {
          return host.readFile(path, encoding);
        },
        write: function(s) {
          return host.write(s);
        },
      ...
      function hasEntry(entries, name) {
        return some(entries, function(file) {
          return fileNameEqual(file, name);
        });
      }
      function updateFileSystemEntry(entries, baseName, isValid) {
        if (hasEntry(entries, baseName)) {
          if (!isValid) {
            return filterMutate(entries, function(entry) {
              return !fileNameEqual(entry, baseName);
            });
          }
        } else if (isValid) {
      ...
      function getNodeSystem() {
        ...
        function createWatchedFileSet() {
          ...
          function addDirWatcher(dirPath) {
            ...
            watcher = fsWatchDirectory(dirPath || ".", function(eventName, relativeFileName) {
          ...
          function fileEventHandler(eventName, relativeFileName, baseDirPath) {
            var fileName = !ts.isString(relativeFileName) ? undefined : ts.getNormalizedAbsolutePath(relativeFileName, baseDirPath);
        ...
        function fsWatchDirectory(directoryName, callback, recursive) {
          var options;
          var watcher = !directoryExists(directoryName) ? watchMissingDirectory() : watchPresentDirectory();
          return {close: function() {
              watcher.close();
            }};
  ...
  (function(ts) {
    ...
    function isJSDocIndexSignature(node) {
      return ts.isTypeReferenceNode(node) && ts.isIdentifier(node.typeName) && node.typeName.escapedText === "Object" && node.typeArguments && node.typeArguments.length === 2 && (node.typeArguments[0].kind === 136 || node.typeArguments[0].kind === 133);
    }
    ...
    function getSourceOfAssignment(node) {
      return ts.isExpressionStatement(node) && node.expression && ts.isBinaryExpression(node.expression) && node.expression.operatorToken.kind === 58 && node.expression.right;
    }
    ts.getSourceOfAssignment = getSourceOfAssignment;
    function getSingleInitializerOfVariableStatement(node, child) {
      return ts.isVariableStatement(node) && node.declarationList.declarations.length > 0 && (!child || node.declarationList.declarations[0].initializer === child) && node.declarationList.declarations[0].initializer;
    }
    ...
    function getSingleVariableOfVariableStatement(node, child) {
      return ts.isVariableStatement(node) && node.declarationList.declarations.length > 0 && (!child || node.declarationList.declarations[0] === child) && node.declarationList.declarations[0];
    }
    ts.getSingleVariableOfVariableStatement = getSingleVariableOfVariableStatement;
    function getNestedModuleDeclaration(node) {
      return node.kind === 233 && node.body && node.body.kind === 233 && node.body;
    }
    ...
    function getJSDocCommentsAndTags(node) {
      ...
      function getJSDocCommentsAndTagsWorker(node) {
        ...
        if (parent && parent.parent && (getSingleVariableOfVariableStatement(parent.parent, node) || getSourceOfAssignment(parent.parent))) {
    ...
    function getParameterSymbolFromJSDoc(node) {
      ...
      var decl = getSourceOfAssignment(host) || getSingleInitializerOfVariableStatement(host) || getSingleVariableOfVariableStatement(host) || getNestedModuleDeclaration(host) || host;
      if (decl && ts.isFunctionLike(decl)) {
        var parameter = ts.find(decl.parameters, function(p) {
    ...
    function mutateMap(map, newMap, options) {
      var createNewValue = options.createNewValue,
          onDeleteValue = options.onDeleteValue,
          onExistingValue = options.onExistingValue;
  ...
  (function(ts) {
    ...
    function getJSDocTags(node) {
      ...
      if (tags === undefined) {
        node.jsDocCache = tags = ts.flatMap(ts.getJSDocCommentsAndTags(node), function(j) {
    ...
    function getAllJSDocTagsOfKind(node, kind) {
      var tags = getJSDocTags(node);
      return ts.filter(tags, function(doc) {
        return doc.kind === kind;
      });
    }
  ...
  (function(ts) {
    ...
    function isTypeNodeKind(kind) {
      return (kind >= 158 && kind <= 173) || kind === 119 || kind === 133 || kind === 134 || kind === 122 || kind === 136 || kind === 137 || kind === 99 || kind === 105 || kind === 139 || kind === 95 || kind === 130 || kind === 201 || kind === 268 || kind === 269 || kind === 270 || kind === 271 || kind === 272 || kind === 273 || kind === 274;
    }
  ...
  (function(ts) {
    ...
    (function(Parser) {
      ...
      function isStartOfParameter() {
        return token() === 24 || isIdentifierOrPattern() || ts.isModifierKind(token()) || token() === 57 || isStartOfType(true);
      }
      ...
      function parseParameterList(flags) {
        if (parseExpected(19)) {
          ...
          var result = parseDelimitedList(16, flags & 32 ? parseJSDocParameter : function() {
            return parseParameter(!!(flags & 8));
          });
      ...
      function parseLiteralTypeNode(negative) {
        ...
        var expression = token() === 101 || token() === 86 ? parseTokenNode() : parseLiteralLikeNode(token());
        if (negative) {
      ...
      function parseParenthesizedArrowFunctionExpressionHead(allowAmbiguity) {
        ...
        if (!allowAmbiguity && ((token() !== 36 && token() !== 17) || ts.find(node.parameters, function(p) {
          return p.initializer && ts.isIdentifier(p.initializer) && p.initializer.escapedText === "= not found";
        }))) {
          return undefined;
        }
        return node;
      ...
      function tryParseTypeArguments() {
        return token() === 27 ? parseBracketedList(19, parseType, 27, 29) : undefined;
      }
  ...
  (function(ts) {
    ...
    function readConfigFile(fileName, readFile) {
      ...
      return ts.isString(textOrDiagnostic) ? parseConfigFileTextToJson(fileName, textOrDiagnostic) : {
        config: {},
    ...
    function getCompilerOptionValueTypeString(option) {
      return option.type === "list" ? "Array" : ts.isString(option.type) ? option.type : "string";
    }
    ...
    function getErrorForNoInputFiles(_a, configFileName) {
      var includeSpecs = _a.includeSpecs,
          excludeSpecs = _a.excludeSpecs;
    ...
    function getFileNamesFromConfigSpecs(spec, basePath, options, host, extraFileExtensions) {
      ...
      var filesSpecs = spec.filesSpecs,
          validatedIncludeSpecs = spec.validatedIncludeSpecs,
          validatedExcludeSpecs = spec.validatedExcludeSpecs,
          wildcardDirectories = spec.wildcardDirectories;
      var supportedExtensions = ts.getSupportedExtensions(options, extraFileExtensions);
      if (filesSpecs) {
        for (var _i = 0,
            filesSpecs_1 = filesSpecs; _i < filesSpecs_1.length; _i++) {
      ...
      if (validatedIncludeSpecs && validatedIncludeSpecs.length > 0) {
        for (var _a = 0,
            _b = host.readDirectory(basePath, supportedExtensions, validatedExcludeSpecs, validatedIncludeSpecs, undefined); _a < _b.length; _a++) {
          var file = _b[_a];
  ...
  (function(ts) {
    ...
    (function(JsTyping) {
      ...
      function loadTypesMap(host, typesMapPath) {
        var result = ts.readConfigFile(typesMapPath, function(path) {
          return host.readFile(path);
        });
        if (result.config) {
  ...
  (function(ts) {
    ...
    function nodeModuleNameResolverWorker(moduleName, containingDirectory, compilerOptions, host, cache, jsOnly) {
      ...
      function tryResolve(extensions) {
        ...
        if (!ts.isExternalModuleNameRelative(moduleName)) {
          ...
          var _a = ts.normalizePathAndParts(ts.combinePaths(containingDirectory, moduleName)),
              candidate = _a.path,
              parts = _a.parts;
          ...
          return resolved_2 && toSearchResult({
            resolved: resolved_2,
            isExternalLibraryImport: ts.contains(parts, "node_modules")
          });
        }
      }
    }
    ...
    function loadNodeModuleFromDirectory(extensions, candidate, failedLookupLocations, onlyRecordFailures, state, considerPackageJson) {
      ...
      var _a = considerPackageJson ? getPackageJsonInfo(candidate, "", failedLookupLocations, onlyRecordFailures, state) : {
        packageJsonContent: undefined,
        packageId: undefined
      },
          packageJsonContent = _a.packageJsonContent,
    ...
    function loadModuleFromNodeModulesFolder(extensions, moduleName, nodeModulesFolder, nodeModulesFolderExists, failedLookupLocations, state) {
      ...
      if (packageInfo.found) {
        ...
        var _a = getPackageName(moduleName),
      ...
      var pathAndExtension = loadModuleFromFile(extensions, candidate, failedLookupLocations, !nodeModulesFolderExists, state) || loadNodeModuleFromDirectoryWorker(extensions, candidate, failedLookupLocations, !nodeModulesFolderExists, state, packageJsonContent);
    ...
    function getPackageNameFromAtTypesDirectory(mangledName) {
      ...
      if (withoutAtTypePrefix !== mangledName) {
        return ts.stringContains(withoutAtTypePrefix, mangledScopedPackageSeparator) ? "@" + withoutAtTypePrefix.replace(mangledScopedPackageSeparator, ts.directorySeparator) : withoutAtTypePrefix;
      }
      return mangledName;
    }
      ...
      (function(typingsInstaller) {
        ...
        var NodeTypingsInstaller = (function(_super) {
          ...
          function NodeTypingsInstaller(globalTypingsCacheLocation, typingSafeListLocation, typesMapLocation, npmLocation, throttleLimit, log) {
            ...
            (_this.nodeExecSync = require('@empty').execSync);
                ...
                case "typesRegistry":
                  {
                    ...
                    var response = {
                      kind: server.EventTypesRegistry,
                      typesRegistry: typesRegistry_1
                    };
                    _this.sendResponse(response);
                    break;
                  }
                case "installPackage":
                  {
                    ...
                    if (cwd) {
                      _this.installWorker(-1, [packageName_1], cwd, function(success) {
                        ...
                        var response = {
                          kind: server.EventPackageInstalled,
                          success: success,
                          message: message
                        };
                        _this.sendResponse(response);
